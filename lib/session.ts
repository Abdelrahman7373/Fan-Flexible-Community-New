...
export const authOptions: NextAuthOptions = {
    ...
    jwt:{
        encode: ({secret, token}) => {
            const encodedwebToken = jsonwebtoken.sign({
                ...token, iss: 'grafbase', exp: Math.floor(Date.now()/1000)+60*60,
            },secret);
            return encodedwebToken;
        },
        decode: async ({secret, token}) => {
            const decodedwebToken = jsonwebtoken.verify(token!,secret);
            return decodedwebToken as JWT;
        },
    },
    theme:{
        colorScheme: 'light', logo: '/Logo.png',
    },
    callbacks:{
        ...
        async signIn({user}: {user: AdapterUser | User}) {
            try {
                ...
                console.log(error, 'ERROR_SIGNIN');
                return false;
            }
        },
    }
